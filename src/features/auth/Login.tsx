import { useState } from "react";
import { login } from "./auth.service";
import { useLocation } from "wouter";
import { AuthError } from "./auth.errors";
import Swal from "sweetalert2";
import { useAuthStore } from "./auth.store";

const Login = () => {
    const [dataForm, setDataForm] = useState({ username: '', password: '' });
    const [_, navigate] = useLocation();
    const loginSuccess = useAuthStore(s => s.loginSuccess)


    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        try {
            const data = await login(dataForm.username, dataForm.password)
            loginSuccess(data);
            navigate('/home')
        } catch (error) {
            if (error instanceof AuthError) {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: error.message,
                })
            } else {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Ocurrió un error inesperado',
                })
            }
        }
    }


    return (
        <div className="flex h-fit min-h-screen w-full">
            <div className="w-2/3 flex items-center justify-center bg-gray-200">
                <img className="w-1/2" src="images/login-security.svg" alt="leftSideImage" />
            </div>

            <div className="w-1/3 flex flex-col items-center justify-center">
                <form onSubmit={handleSubmit} className="md:w-100 w-80 flex flex-col items-start justify-center">
                    <h2 className="text-xl text-gray-900 font-bold">Bienvenido a MEG</h2>
                    <p className="text-sm text-gray-500/90 mt-3">Modelo de Entregas Garantizadas</p>

                    <label className="input w-full rounded-lg mt-6">
                        <svg width="16" height="11" viewBox="0 0 16 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fillRule="evenodd" clipRule="evenodd" d="M0 .55.571 0H15.43l.57.55v9.9l-.571.55H.57L0 10.45zm1.143 1.138V9.9h13.714V1.69l-6.503 4.8h-.697zM13.749 1.1H2.25L8 5.356z" fill="#6B7280" />
                        </svg>
                        <input
                            type="text"
                            className="grow"
                            placeholder="Usuario"
                            onChange={(e) => {
                                const t = e.target as HTMLInputElement;
                                setDataForm({ ...dataForm, username: t.value });
                            }}
                            onKeyDown={(e) => {
                                if (e.key === "Enter") {
                                    handleSubmit(e);
                                }
                            }}
                        />
                    </label>

                    <label className="input w-full rounded-lg mt-6">
                        <svg width="13" height="17" viewBox="0 0 13 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M13 8.5c0-.938-.729-1.7-1.625-1.7h-.812V4.25C10.563 1.907 8.74 0 6.5 0S2.438 1.907 2.438 4.25V6.8h-.813C.729 6.8 0 7.562 0 8.5v6.8c0 .938.729 1.7 1.625 1.7h9.75c.896 0 1.625-.762 1.625-1.7zM4.063 4.25c0-1.406 1.093-2.55 2.437-2.55s2.438 1.144 2.438 2.55V6.8H4.061z" fill="#6B7280" />
                        </svg>
                        <input
                            type="password"
                            className="grow"
                            placeholder="Contraseña"
                            onChange={(e) => {
                                const t = e.target as HTMLInputElement;
                                setDataForm({ ...dataForm, password: t.value });
                            }}
                            onKeyDown={(e) => {
                                if (e.key === "Enter") {
                                    handleSubmit(e);
                                }
                            }} />
                    </label>

                    <button type="submit" className="mt-8 w-full h-11 rounded-lg text-white bg-indigo-500 hover:opacity-90 transition-opacity">
                        Iniciar Sesión
                    </button>

                    <p className="w-full text-center text-sm text-gray-600/90 mt-5">Distribuciones Pharmaser LTDA.</p>
                </form>
            </div>
        </div>
    );
};

export default Login;
